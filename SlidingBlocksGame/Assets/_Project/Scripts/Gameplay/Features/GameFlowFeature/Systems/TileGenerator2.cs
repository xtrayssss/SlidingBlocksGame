...
namespace _Project.Scripts.Gameplay.Features.GameFlowFeature.Systems
{
    public class TileGenerator2 : MonoBehaviour
    {
        ...
        {
            Vector3 startPosition = new Vector3(columns * tileSpacing / 2, 0, rows * tileSpacing / 2);

            // Создание игрового поля плиток в одной точке
            for (int row = 0; row < rows; row++)
            {
                for (int col = 0; col < columns; col++)
                {
                    ...
                    StartCoroutine(GrowTile(tile, startPosition, new Vector3(col * tileSpacing, 0, row * tileSpacing)));
