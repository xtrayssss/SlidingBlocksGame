...
{
    public class TileGenerator5 : MonoBehaviour
    {
        ...
        {
            // Создание игрового поля плиток по линиям
            for (int row = 0; row < rows; row++)
            {
                Vector3 startPosition = new Vector3(columns * tileSpacing / 2, 0, row * tileSpacing);

                for (int col = 0; col < columns; col++)
                {
                    Vector3 endPosition = new Vector3(col * tileSpacing, 0, row * tileSpacing);
                    GameObject tile = Instantiate(tilePrefab, startPosition, Quaternion.identity);
                    tile.transform.localScale = Vector3.zero;
                    tiles.Add(tile); // Добавление плитки в список
                    StartCoroutine(GrowTile(tile, startPosition, endPosition));
                    yield return new WaitForSeconds(growthDuration / columns);
                }

                // Ожидание завершения анимации роста и перемещения текущей линии перед переходом к следующей
                yield return new WaitForSeconds(growthDuration + moveDuration);
            }
        }

        private IEnumerator GrowTile(GameObject tile, Vector3 startPosition, Vector3 targetPosition)
        {
            ...
            // Анимация перемещения плитки к конечному положению
            while (elapsedTime < moveDuration)
            {
                elapsedTime += Time.deltaTime;
                float waveOffset = Mathf.Sin((targetPosition.x + targetPosition.z) * waveFrequency) * waveAmplitude;
                Vector3 wavePosition = new Vector3(targetPosition.x, waveOffset, targetPosition.z);
                tile.transform.position = Vector3.Lerp(startPosition, wavePosition, elapsedTime / moveDuration);
                yield return null;
            }
