...
namespace _Project.Scripts.Gameplay.Features.ScrollFeature
{
    ...
    public class ScrollSnap : MonoBehaviour
    {
        ...
        private void UpdateAll()
        {
            ...
            {
                _lastScrollPos = scrollbar.value;
                _hasStartedScrolling = false; // Сброс флага
            }

            if (Input.GetMouseButton(0))
            {
                // Проверяем, изменился ли скролл и не было ли уже вызвано событие
                if (!_hasStartedScrolling && Mathf.Abs(scrollbar.value - _lastScrollPos) > 0.01f)
                {
                    OnScrollStarted?.Invoke(_selectedItemIndex, _items[_selectedItemIndex]);
                    _hasStartedScrolling = true; // Устанавливаем флаг, чтобы событие больше не вызывалось
                }
